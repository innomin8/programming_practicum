...
class Level():
    ...
    def load_enemies(self):
        for enemy_info in self.enemies:
            ...
            area = pygame.Rect(x_min, y, x_max - x_min, win_height - y)  # Область патрулирования

            if enemy_type == 0:
                enemy = Enemy1(game.costumes[gr_enemy], x, y, area, x_speed)
                enemy.start_x = x
                enemy.patrol_distance = (x_max - x_min) // 2
            elif enemy_type == 1:
                enemy = Enemy2(game.costumes[gr_enemy2], x, y, area, x_speed)
                enemy.start_x = x
                enemy.patrol_distance = (x_max - x_min) // 2
            elif enemy_type == 2:
                enemy = Enemy3(game.costumes[gr_enemy3], x, y, area, x_speed)
                enemy.start_x = x
                enemy.patrol_distance = (x_max - x_min) // 2
